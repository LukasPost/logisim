...
namespace draw.shapes
{
	...
	public class CurveUtil
	{
		...
		private const double zeroMax = 0.0000001;
		...
		public static Bounds getBounds(double[] p0, double[] p1, double[] p2)
		{
			...
			int x = (int) xMin;
			int y = (int) yMin;
			int w = (int) Math.Ceiling(xMax) - x;
			int h = (int) Math.Ceiling(yMax) - y;
		...
		public static double[] findNearestPoint(double[] q, double[] p0, double[] p1, double[] p2)
		{
			...
			double[] pos = new double[] {p0[0] - q[0], p0[1] - q[1]};
			...
			double dist2Min = double.MaxValue;
		...
		private static double[] solveCubic(double a, double b, double c, double d)
		{
			if (Math.Abs(a) > zeroMax)
			{
				...
				if (D > zeroMax)
				{
					...
					double u = (-q + z) / 2;
					double v = (-q - z) / 2;
					u = (u >= 0) ? Math.Pow(u, 1.0 / 3) : -Math.Pow(-u, 1.0 / 3);
					v = (v >= 0) ? Math.Pow(v, 1.0 / 3) : -Math.Pow(-v, 1.0 / 3);
				...
				else if (D < -zeroMax)
				{
					...
					return new double[] {u * Math.Cos(v) + offset, u * Math.Cos(v + 2 * Math.PI / 3) + offset, u * Math.Cos(v + 4 * Math.PI / 3) + offset};
				}
				else
				{
					// D zero
