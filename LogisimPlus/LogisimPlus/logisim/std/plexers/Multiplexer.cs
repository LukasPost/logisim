...
namespace logisim.std.plexers
{
	...
	public class Multiplexer : InstanceFactory
	{
		...
		private void updatePorts(Instance instance)
		{
			...
			for (int i = 0; i < inputs; i++)
			{
				ps[i].ToolTip = Strings.getter("multiplexerInTip", "" + i);
			}
			ps[inputs].ToolTip = Strings.getter("multiplexerSelectTip");
			if (enable)
			{
				ps[inputs + 1].ToolTip = Strings.getter("multiplexerEnableTip");
			}
			ps[ps.Length - 1].ToolTip = Strings.getter("multiplexerOutTip");
		...
		public override void paintInstance(InstancePainter painter)
		{
			...
			JGraphicsUtil.drawCenteredText(g, "MUX", bds.X + bds.Width / 2, bds.Y + bds.Height / 2);
			painter.drawPorts();
		}

		internal static void drawSelectCircle(JGraphics g, Bounds bds, Location loc)
		{
