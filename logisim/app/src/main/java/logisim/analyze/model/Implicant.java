...
public class Implicant implements Comparable<Implicant> {
	...
	static List<Implicant> MINIMAL_LIST = List.of(MINIMAL_IMPLICANT);
	...
	private static class TermIterator implements Iterable<Implicant>, Iterator<Implicant> {
		...
		public @NotNull Iterator<Implicant> iterator() {
			return this;
		}
	...
	public boolean equals(Object other) {
		return other instanceof Implicant o && unknowns == o.unknowns && values == o.values;
	}
	...
	static List<Implicant> computeMinimal(int format, AnalyzerModel model, String variable) {
		...
		while (!toCover.isEmpty()) {
			...
			if (max != null) {
				retSet.add(max);
				primes.remove(max);
				for (Implicant term : max.getTerms()) toCover.remove(term);
			}
		}
