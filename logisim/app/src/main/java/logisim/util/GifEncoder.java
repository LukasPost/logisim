...
public class GifEncoder {
	...
	private static class LZWStringTable {
		...
		void ClearTable(int codesize) {
			...
			for (int q = 0; q < HASHSIZE; q++) strHsh_[q] = HASH_FREE;
	...
	private static class MyGrabber extends PixelGrabber {
		...
		public void setPixels(int srcX, int srcY, int srcW, int srcH, ColorModel model, int[] pixels, int srcOff,
			...
			if (monitor.isCanceled()) abortGrabbing();
			else super.setPixels(srcX, srcY, srcW, srcH, model, pixels, srcOff, srcScan);
		}
	}
	 ...
	 * @exception AWTException Will be thrown if the pixel grab fails. This can happen if Java runs out of memory. It
	 *                         may also indicate that the image contains more than 256 colors.
	 */
	public GifEncoder(Image image, ProgressMonitor monitor) throws AWTException {
		...
		if (monitor != null) grabber = new MyGrabber(monitor, image, 0, 0, width_, height_, values, 0, width_);
		else grabber = new PixelGrabber(image, 0, 0, width_, height_, values, 0, width_);
