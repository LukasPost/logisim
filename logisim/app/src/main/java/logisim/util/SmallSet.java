...
public class SmallSet<E> extends AbstractSet<E> {
		...
		public void remove() {
			if (itVersion != version) throw new ConcurrentModificationException();
				...
				if (size == 2) {
					...
					for (int i = pos; i < size; i++) vals[i - 1] = vals[i];
					--pos;
					--size;
					vals[size] = null;
				}
	...
	public Object @NotNull [] toArray() {
		Object vals = values;
		int sz = size;
		if (sz == 1) return new Object[]{vals};
	...
	public boolean isEmpty() {
		if (size <= HASH_POINT) return size == 0;
		else return ((HashSet<?>) values).isEmpty();
	}

	@Override
	public int size() {
		if (size <= HASH_POINT) return size;
		else return ((HashSet<?>) values).size();
	}
	...
	public boolean add(E value) {
		...
		if (oldSize < 2) if (oldSize == 0) {
			...
			if (oldValues.equals(value)) return false;
			else {
			...
			if (oldSize < HASH_POINT) vals[oldSize] = value;
			else {
	...
	public boolean contains(Object value) {
		if (size <= 2) return size != 0 && values.equals(value);
		else if (size <= HASH_POINT) {
			Object[] vals = (Object[]) values;
	...
	public @NotNull Iterator<E> iterator() {
		if (size <= HASH_POINT) if (size == 0) return IteratorUtil.emptyIterator();
	...
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			...
			for (String s : set) System.out.print(" " + s); // OK
